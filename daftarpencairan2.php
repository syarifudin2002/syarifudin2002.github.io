...
foreach ($tables as $table) {
    ...
    $query = "SELECT Nomor_Registrasi, Jenis_Penerima, Nama_Lembaga, NIK_Pimpinan, Nama_Pimpinan, Alamat, Jenis_Dana, Via, Jumlah_Dana, Keterangan
          FROM $table   
...
$query_get_pencairan = "SELECT * FROM pencairank";
...
<body>
                        ...
                        if (!empty($display_data)) {
                            foreach ($display_data as $data) {
                                ...
                                <tr class="data-row">
                                    ...
                                    <td>
                                    <a href="#" onclick="printKwitansi('<?php echo htmlspecialchars($data['NIK_Pimpinan']); ?>')" class="btn btn-primary btn-sm">Cetak</a>
                                        <a href="delete2.php?NIK_Pimpinan=<?php echo htmlspecialchars($data['NIK_Pimpinan']); ?>&origin=daftarpencairan2" onclick="return confirm('Anda yakin ingin menghapus data ini?')" class="btn btn-danger btn-sm">Delete</a>
                                    </td>
                                </tr>
                            <?php
                            }
    ...
    <script>
         function printKwitansi(nikpimpinan) {
            // Buat URL untuk halaman cetak
            var url = 'cetak2.php?NIK_Pimpinan=' + encodeURIComponent(nikpimpinan);
